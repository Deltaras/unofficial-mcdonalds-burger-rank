...
import { DndContext, DragEndEvent, DragOverlay, PointerSensor, closestCorners, useDroppable, useDraggable, useSensor, useSensors } from '@dnd-kit/core'
...
export default function TierListApp() {
  ...
  useEffect(() => { try { const saved = localStorage.getItem('mcd-burger-tierlist-v1'); if (saved) setBoard(cleanBoard(JSON.parse(saved))) } catch {} }, [])
  useEffect(() => { localStorage.setItem('mcd-burger-tierlist-v1', JSON.stringify(board)) }, [board])
  ...
  async function exportPng() {
    const node = document.getElementById('tier-board'); if (!node) return
    const data = await toPng(node, { pixelRatio: 2, cacheBust: true, backgroundColor: '#171717' })
    const link = document.createElement('a'); link.download = 'mein-mcdonalds-tier-list.png'; link.href = data; link.click()
  }
  return <main className="app-shell">
    <header className="hero"><div className="brand-mark"><Utensils size={23} /></div><div><p className="eyebrow">McDonald&apos;s Deutschland</p><h1>BURGER <em>TIER LIST</em></h1></div><div className="hero-badge">DEIN RANKING<br /><b>2026</b></div></header>

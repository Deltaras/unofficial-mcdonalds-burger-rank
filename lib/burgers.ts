...
export const burgers: Burger[] = [
  ...
  ['big-tasty-pepper-bacon', 'Big Tasty Pepper Bacon', 'Pfeffrig & rauchig', 'big-tasty-pepper-bacon.png'],
  ...
  ['mccrispy-pepper-delight', 'McCrispy Pepper Delight', 'Knusprig & pfeffrig', 'mccrispy-pepper-delight.png'],
  ['mccrispy', 'McCrispy', 'Knuspriges Hähnchen', 'mccrispy.png'],
  ['double-royal-ts', 'Double Hamburger Royal TS', 'Doppelt mit Tomate & Salat', 'double-royal-ts.png'],
  ['double-royal-kaese', 'Double Hamburger Royal Cheese', 'Doppelt extra cheesy', 'double-royal-kaese.png'],
  ...
  ['mcdouble-chili-cheese', 'McDouble Chili Cheese', 'Doppelt mit Chili-Käse', 'mcdouble-chili-cheese.png'],
  ...
  ['double-chickenburger', 'Double Chickenburger', 'Doppelt knuspriges Hähnchen', 'chickenburger.png'],
...
export const defaultBoard: BoardState = { pool: burgers.map((b) => b.id), S: [], A: [], B: [], C: [], D: [] }
export const burgerById = Object.fromEntries(burgers.map((b) => [b.id, b]))

export function getTier(id: string, board: BoardState) {
  return (Object.keys(board) as (keyof BoardState)[]).find((tier) => board[tier].includes(id)) ?? 'pool'
}
...
export function cleanBoard(value: unknown): BoardState {
  ...
  for (const tier of Object.keys(result) as (keyof BoardState)[]) {
    for (const id of Array.isArray(raw[tier]) ? raw[tier] : []) {
      if (typeof id === 'string' && valid.has(id) && !used.has(id)) { result[tier].push(id); used.add(id) }
    }
  }
